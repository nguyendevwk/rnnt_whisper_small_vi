...
class AudioDataset(Dataset):
    def __init__(self,
                 ...
                 base_path,  # Nhận base_path từ bên ngoài
                 bg_noise_path=None,  # Tắt background noise mặc định
                 shuffle=False,
                 augment=False,
                 max_duration=15.1,
                 min_duration=0.9,
        ...
        self.samples = []
        throw_away = 0
        self.base_path = base_path  # Lưu base_path từ tham số

        # Đọc manifest và kiểm tra tệp âm thanh
        for manifest_file in manifest_files:
        ...
        self.tokenizer = spm.SentencePieceProcessor(model_file=tokenizer_model_path)
        self.device = 'cpu'  # Dùng CPU để tránh lỗi CUDA không cần thiết

        # Tắt augmentation để ưu tiên tập train
        self.augmentation = lambda samples, sample_rate: samples if not augment else samples
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            logger.warning(f"Failed to load {audio_path}: {str(e)}. Skipping.")
            # Trả về mẫu tiếp theo nếu lỗi
